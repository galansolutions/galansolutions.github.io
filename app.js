...
function initForm(){
  ...
  // Conservative, bounds-checked validation (defence in depth on top of the
  // HTML required/maxlength attributes). We never trust the client, but this
  // keeps obviously-bad payloads from being sent and gives users clear feedback.
  ...
  form.addEventListener('submit',async function(e){
    ...
    // Honeypot — real users never see this field. If filled, fake success and bail.
    ...
    // A Google Form ID is NOT a secret — it is public by design once the form is
    // shared. It is, however, an abusable write endpoint: meaningful anti-spam
    // and rate-limiting require a backend/serverless proxy (see SECURITY.md).
    ...
    // Abort if the network hangs rather than leaving the request pending forever.
